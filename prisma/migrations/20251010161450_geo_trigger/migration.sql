...
CREATE OR REPLACE FUNCTION update_hazard_geolocation() RETURNS TRIGGER AS $$
BEGIN
  -- Only update geoLocation if both latitude and longitude are present
  IF NEW.latitude IS NOT NULL AND NEW.longitude IS NOT NULL THEN
    -- Create a PostGIS POINT geometry from longitude and latitude
  ...
  ELSE
    -- If either coordinate is NULL, set geoLocation to NULL
...
CREATE TRIGGER trigger_update_hazard_geolocation 
  BEFORE INSERT OR UPDATE ON "Hazard" 
  FOR EACH ROW 
  EXECUTE FUNCTION update_hazard_geolocation();
