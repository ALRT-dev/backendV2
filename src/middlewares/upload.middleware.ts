...
/** Use after optionalCategoryImagesUpload for category create/update routes (5MB limit, 12 files). */
export const handleCategoryImagesMulterError = (
  ...
  if (error instanceof multer.MulterError) {
    switch (error.code) {
      ...
      case "LIMIT_FILE_COUNT":
        return next(
          new HttpError(400, "Too many category images. Maximum 12 files allowed.")
        );
      case "LIMIT_UNEXPECTED_FILE":
        return next(new HttpError(400, "Unexpected file field."));
      default:
